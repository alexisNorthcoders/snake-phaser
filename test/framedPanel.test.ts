...
test('frame: shadow, outline, scrim with given alpha, four 4px #333333 border strips', () => {
  ...
  assert.deepEqual(r[0], { x: 0, y: 4, w: 560, h: 480, color: 0, alpha: 1 });
  assert.deepEqual(r[1], { x: 0, y: 0, w: 560, h: 480, color: 0, alpha: 1 });
  assert.deepEqual(r[2], { x: 4, y: 4, w: 552, h: 472, color: 0, alpha: 0.9 });
  assert.equal(r.length, 7);
  assert.ok(r.slice(3).every((b) => b.color === 0x333333 && (b.w === 4 || b.h === 4)));
});
