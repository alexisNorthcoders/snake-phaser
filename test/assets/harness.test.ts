...
test('the background prompt is the description alone, with no style words', () => {
  assert.equal(buildBackgroundPrompt(theme), theme.background.description)
})

test('every theme names a background and covers every food type', async () => {
...
test('contact sheet lists numbered background candidates, linking the shipped pixels', () => {
  const html = renderContactSheet('t', [], [], [2, 1])
  assert.match(html, /background #1/)
  assert.match(html, /src="background\/background-2\.png" width="200"/)
  assert.match(html, /href="raw\/background-2\.png"/)
  assert.ok(html.indexOf('background #1') < html.indexOf('background #2'))
})
